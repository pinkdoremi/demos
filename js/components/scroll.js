...
(function($) {
  ...
  function Scroll(el, options) {
    ...
    this.translateZ = utils.hasPerspective && this.options.HWCompositing ?
      ' translateZ(0)' : '';
  ...
  Scroll.prototype = {
    ...
    _initEvents: function(remove) {
      ...
      if (utils.hasPointer && !this.options.disablePointer) {
        eventType(this.wrapper, utils.prefixPointerEvent('pointerdown'),
          this);
    ...
    refresh: function() {
      ...
      this.scrollerWidth = this.scroller.offsetWidth + parseInt(pl) +
        parseInt(pr) + parseInt(ml2) + parseInt(mr2);
      this.scrollerHeight = this.scroller.offsetHeight + parseInt(pt) +
        parseInt(pb) + parseInt(mt2) + parseInt(mb2);
    ...
    _start: function(e) {
      ...
      if (this.options.useTransition && this.isInTransition && this.options
        .role !== 'slider' && this.options.role !== 'tab') {
    ...
    _move: function(e) {
      ...
      if (timestamp - this.endTime > 300 && (absDistX < 10 && absDistY <
          10)) {
        return;
      }
      ...
      if (e.changedTouches[0].clientX > window.innerWidth - 5 || e.changedTouches[0].clientX < 5 || e.changedTouches[0].clientY >
        window.innerHeight - 5 || e.changedTouches[0].clientY < 5) { //抵达手机边缘时，强制触发结束
        this._end(e);
      }
    },
    ...
    _end: function(e) {
      ...
      if (this.options.preventDefault && !utils.preventDefaultException(e
          .target, this.options.preventDefaultException)) {
        e.preventDefault();
      }
      ...
      if (this.resetPosition(this.options.bounceTime)) { // reset if we are outside of the boundaries
        if (this.options.role === 'tab') {
          $(this.scroller.children[this.currentPage]).siblings('li').height(
            0);
        }
        return;
      }
      ...
      if (newX != this.x || newY != this.y) {
        ...
        if (newX > 0 || newX < this.maxScrollX || newY > 0 || newY < this
          .maxScrollY) {
      ...
      // tab
      // ==========================
      if (this.options.role === 'tab' && $(event.target).closest('ul').hasClass(
          'nav')) {
      ...
      if (this.options.role === 'slider' || this.options.role === 'tab') {
        ...
        if (distanceX < 30) {
          this.scrollTo(-this.itemWidth * this.currentPage, 0, this.options
            .bounceTime, this.options.bounceEasing);
        } else if (newX - this.startX < 0) { // 向前
          this._execEvent('beforeScrollStart', this.currentPage, this.currentPage +
            1);
          this.scrollTo(-this.itemWidth * ++this.currentPage, 0, this.options
            .bounceTime, this.options.bounceEasing);
    ...
    scrollTo: function(x, y, time, easing) {
      ...
      if (!time || (this.options.useTransition && easing.style)) {
        ...
        if (this.options.role === 'slider' || this.options.role === 'tab') { // 不添加判断会影响 left/top 的过渡
          ...
          this.scrollerStyle[utils.style.transitionProperty] = utils.style
            .transform;
        }
        ...
        var startTime = new Date,me = this;
        if(time){
    ...
    _autoplay: function() {
      ...
      self.scrollTo(-self.itemWidth * self.currentPage, 0, self.options.bounceTime,
  ...
  window.tc.Scroll = window.frozen.Scroll = Scroll;
